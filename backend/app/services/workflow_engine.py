...
class WorkflowEngine:
    ...
    def _execute_automation(instance_id, step, context):
        ...
        try:
            ...
            if not automation or automation.get('script_id') is None:
                ...
                raise ValueError(f"Step {step['id']} is missing automation configuration")
            ...
            try:
                ...
                automation_config = {
                    ...
                    'timeout': automation.get('timeout', script_data.get('timeout', 60)),
                    'allow_network': script_data.get('allow_network', False),
                    'retry_attempts': automation.get('retry_attempts', 3),
                    'retry_on_error': automation.get('retry_on_error', False),
                }
            except Exception as e:
                logger.error(f"Failed to load script {script_id}: {e}")
                raise

            if not automation_config.get('script_content'):
            ...
            # Retry handling
            attempts = 0
            max_attempts = automation_config.get('retry_attempts', 1)
            while attempts < max_attempts:
                attempts += 1
                logger.info(f"Attempt {attempts}/{max_attempts} for automation step {step['id']}")
    ...
    #     try:
    #         properties = step.get('properties', {})
    #         # logger.info(f"Ameir Elshareif : properties is : {step}")
    #         # Get automation configuration from step properties
    #         automation = properties.get('automation', {})
            
    #         if not automation or automation.get('script_id')==None:
    #             logger.warning(f"No automation configuration for step {step['id']}")
    #             raise ValueError(f"Step {step['id']} is missing automation configuration")
    ...
    #                     'type': script_data.get('type', 'script_execution'),
    #                     'script_type': script_data.get('script_type', 'python'),
    #                     'script_content': script_data.get('script_content'),  # Pass actual content
    #                     'script_id': script_id,
    #                     'script_name': script_data.get('name', 'Unnamed Script'),
    #                     'timeout': automation.get('timeout', 300),
    #                     'allow_network': script_data.get('allow_network', False),
    ...
    #             not automation_config.get('script_content')):
    #             raise ValueError(f"Script execution step {step['id']} requires script content")
